...
# Key information about version 1 of `elevatr`
...
There are currently several endpoints that `elevatr` accesses.  For point elevation data it uses the USGS Elevation Point Query Service and can also access point elevations from the Amazon Web Services (AWS) Terrain Tiles.  To access raster elevation data (e.g. a DEM) it uses both the AWS terrain tiles as well as OpenTopography's GL3, GL1, ALOS, and Global Bathymetry/Topography (see <https://opentopography.org/developers> for more details).   
