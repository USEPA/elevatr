...
```{r get_raster}
# Key is required here for vignette build.
# If following advice above in the API Keys section this is not required.
key <- readRDS("../tests/testthat/key_file.rds")

# SpatialPolygonsDataFrame example
data(lake)
elevation <- get_elev_raster(lake,z = 9, api_key = key)
plot(elevation)
plot(lake, add=TRUE)

# data.frame example
elevation_df <- get_elev_raster(examp_df,prj=prj_dd, z = 5, api_key = key)
plot(elevation_df)
